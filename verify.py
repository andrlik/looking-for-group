...
print(
    "Welcome to the Keybase verification script. The point of this script "
...
# CHANGE ME
# ---------
identity_service_username = "daniel"
identity_service_password = "rabid.24"
...
kb_username = "t_alice"
sig_id = "8514ae2f9083a3c867318437845855f702a4154d1671a19cf274fb2e6b7dec7c0f"
...
try:
    ...
    print(
        "Missing path to config. Please run like this: `python verify.py ./config.json`"
    )
    sys.exit()
...
if identity_service_username == "squeekytween":
    identity_service_username = input(
        f"Username of a valid account in {config['display_name']}: "
    )
...
# )[0]
# print("Found csrf token of {}".format(authenticity_token))
data = {
    "login": identity_service_username,
...
print(
    f"2. Created an authenticated session to {config['display_name']} for {identity_service_username}."
)
...
print(
    "3. Was able to GET the prefill_url, and naively, the response appears "
...
print(
    f"4. Successfully posted the proof for {kb_username} to {config['display_name']}!"
)
