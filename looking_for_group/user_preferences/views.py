...
class Dashboard(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['friend_requests'] = social_models.GamerFriendRequest.objects.filter(status='new').select_related('requestor')
        player_q = Q(id__in=[g.id for g in game_models.Player.objects.filter(gamer=gamer).exclude(game__status__in=['closed', 'cancel'])])
        gm_q = Q(gm=gamer)
        context['gamer_active_games'] = game_models.GamePosting.objects.filter(player_q | gm_q).select_related('event').prefetch_related('gamesession_set')
        context['gamer_communities'] = social_models.CommunityMembership.objects.filter(gamer=gamer).select_related('community')
        next_sessions = []
        ...
        context['pending_community_applications'] = social_models.CommunityApplication.objects.filter(gamer=gamer, status='pending')
        context['pending_game_applications'] = game_models.GamePostingApplication.objects.filter(gamer=gamer, status='pending')
        context['games_applicants'] = game_models.GamePostingApplication.objects.filter(game__in=game_models.GamePosting.objects.filter(gm=gamer)).filter(status='pending')
        comm_admin = social_models.CommunityMembership.objects.filter(gamer=gamer, community_role='admin').select_related('community').prefetch_related('community__communityapplication_set')
        communities_and_applicant_count = {}
        if comm_admin.count() > 0:
            for comm in comm_admin:
                if comm.community.communityapplication_set.filter(status='pending').count() > 0:
                    communities_and_applicant_count[comm.community.name] = {'community_obj': comm.community, 'pending_applicant_count': comm.community.communityapplication_set.filter(status='pending').count()}
        context['community_applicant_counts'] = communities_and_applicant_count
        context['site_total_communities'] = cache.get_or_set('site_total_communities', social_models.GamerCommunity.objects.count())
        context['site_total_gamers'] = cache.get_or_set('site_total_gamers', social_models.GamerProfile.objects.count())
        context['site_total_games'] = cache.get_or_set('site_total_games', game_models.GamePosting.objects.count())
        context['site_total_active_games'] = cache.get_or_set('site_total_active_games', game_models.GamePosting.objects.exclude(status__in=['cancel', 'closed']).count())
